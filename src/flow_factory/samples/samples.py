...
# src/flow_factory/samples/samples.py
...
class BaseSample:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> BaseSample:
        ...
        if conflicting_keys:
            raise ValueError(f"extra_kwargs contains reserved field names: {conflicting_keys}")
    ...
    def copy_and_replace(self, **overrides: Any) -> "BaseSample":
        """Shallow-copy this sample with safe handling of mutable fields.

        - Fields passed in *overrides* are set to the given values (no copying).
        - ``dict`` / ``list`` / ``set`` values (including ``extra_kwargs``) are
          shallow-copied so the container is not shared while nested objects
          stay referenced.
        - Remaining values (tensors, scalars, ``None``) are copied by reference.

        Unknown field names raise ``TypeError``.

        Risks / caveats:
            - **Shallow containers**: nested dicts/objects inside a copied
              ``list``/``dict``/``set`` are still **shared** with the original;
              in-place mutation of those inner objects affects both samples.
            - **Tensors** are referenced, not cloned; in-place ops on shared
              tensors mutate both sides.
            - New subclass fields that are **mutable but not** ``dict`` /
              ``list`` / ``set`` (e.g. custom mutable objects) are copied by
              **reference** like tensors—two samples may still alias unless
              the subclass overrides or supplies *overrides*.
        """
        # Risks (see docstring): shallow copy still shares nested mutables;
        # tensors and other non-(dict, list, set) mutables are aliased by reference.
        field_names = {f.name for f in fields(self)}
        bad = set(overrides) - field_names
        if bad:
            raise TypeError(f"Unknown fields for {type(self).__name__}: {bad}")
...
class ImageConditionSample(BaseSample):
    ...
    condition_images: Optional[
        ImageBatch
    ] = None  # A list of (Image.Image | torch.Tensor | np.ndarray) or a batched tensor/array
...
class VideoConditionSample(BaseSample):
    ...
    condition_videos: Optional[
        VideoBatch
    ] = None  # A list of (List[Image.Image] | torch.Tensor | np.ndarray) or a batched tensor/array
