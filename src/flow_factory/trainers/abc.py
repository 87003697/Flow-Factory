...
class BaseTrainer(ABC):
    ...
    def _extra_eval_inference_kwargs(self) -> dict:
        ...
        Override in subclasses to inject model-specific evaluation parameters.
        For example, Trellis2 overrides this to pass render kwargs and
        multi-stage pipeline flags (stages=["dense", "shape", "tex"]).
        """
        return {}
    ...
    def evaluate(self) -> None:
        ...
        with torch.no_grad(), self.autocast(), self.adapter.use_ema_parameters():
            for dataset_name, dataloader in self.eval_dataloaders.items():
                ...
                eval_kwargs.update(self._extra_eval_inference_kwargs())
