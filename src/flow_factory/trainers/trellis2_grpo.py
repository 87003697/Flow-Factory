...
Extends GRPOTrainer to share deterministic upstream stages (dense, shape)
across ranks via an owner-broadcast protocol, enabling load-balanced
rollouts with any sampler topology.
"""
...
class Trellis2GRPOTrainer(GRPOTrainer):
    ...
    def sample(self) -> List[BaseSample]:
        ...
        with torch.no_grad(), self.autocast():
            for window_idx in tqdm(
                ...
                sample_batch = self._rollout_group(merged_batch, trajectory_indices)
    ...
    def _distributed_upstream_stages(
        ...
        pilot_results = self._run_owned_pilots(
            samples, merged_batch, resolution, trajectory_indices, uid_to_owner,
        )

        for uid in sorted(uid_to_owner):
            self._broadcast_upstream_for_uid(
                uid, uid_to_owner[uid], pilot_results, samples,
            )
    ...
    def _create_sample_stubs(
        ...
        return [
            Trellis2Sample(
                condition_images=(
                    cond_images[b] if cond_images is not None else None
                ),
    ...
    def _run_owned_pilots(
        ...
        my_owned_uids = [
            uid for uid, owner in uid_to_owner.items() if owner == my_rank
        ]
    ...
    def _broadcast_tensor(
        ...
        if my_rank == src:
            t = (
                ...
                if dtype else tensor.to(device=device).contiguous()
            )
            ndim_t[0] = t.ndim
        ...
        if my_rank != src:
            t = torch.zeros(
                *shape_t.tolist(), dtype=dtype or torch.float32, device=device,
            )
    ...
    def _broadcast_upstream_for_uid(
        ...
        coords = self._broadcast_tensor(
            pilot.sparse_coords if is_owner else None, owner, dtype=torch.int32,
        )
        dense_latent = self._broadcast_tensor(
            pilot.dense_final_latent if is_owner else None, owner,
        )

        shape_latent = None
        if 'shape' in self._upstream_stages:
            shape_latent = self._broadcast_tensor(
                pilot.shape_final_latent if is_owner else None, owner,
            )
        ...
        for s in local_samples:
            ...
            # Upstream metadata (dense_image_cond, dense_all_latents, …) is
            # intentionally NOT copied here.  Only the broadcast fields are
            # required for the downstream training stage; copying metadata
            # only on the owner would create a None/Tensor mismatch when
            # optimize() stacks owner and non-owner samples together.
