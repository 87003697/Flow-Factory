...
class Arguments(ArgABC):
    ...
    def _align_batch_geometry(self) -> None:
        ...
        elif sampler_type == "distributed_group_aligned":
            self._align_for_distributed_group_aligned()
        else:
            raise ValueError(
                f"Unknown sampler_type={sampler_type!r}; "
                "expected one of {'distributed_k_repeat', 'group_contiguous', "
                "'group_distributed', 'distributed_group_aligned'}."
            )
    ...
    def _align_for_distributed_group_aligned(self) -> None:
        """``DistributedGroupAlignedSampler``: ``(num_replicas *
        per_device_batch_size) % group_size == 0`` is required so all K
        copies of every prompt fit in one global iteration; then do the
        base ``unique_sample_num_per_epoch`` alignment.

        Unlike ``GroupDistributedSampler``, this sampler does **not**
        require ``group_size % num_replicas == 0`` — it scatters K copies
        across ranks for load balancing, so configurations with ``K < W``
        (e.g. ``W=8, B=2, K=4``) are perfectly valid.
        """
        ta = self.training_args
        if ta.group_size <= 0:
            raise ValueError(f"group_size must be positive, got {ta.group_size}.")
        ...
        if sample_num_per_iteration % ta.group_size != 0:
            raise ValueError(
                "sampler_type='distributed_group_aligned' requires "
                "`(num_replicas * per_device_batch_size) %% group_size == 0`; "
                f"got num_replicas={world_size}, "
                f"per_device_batch_size={per_device_batch_size}, "
                f"group_size={ta.group_size} "
                f"(W*B={sample_num_per_iteration} % K={ta.group_size} "
                f"= {sample_num_per_iteration % ta.group_size}). "
                "Either bump per_device_batch_size, drop group_size to a "
