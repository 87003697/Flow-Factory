...
class DistributedGroupAlignedSampler(Sampler):
    ...
    Difference from :class:`GroupDistributedSampler`
    -----------------------------------------------
    - This sampler does NOT require ``group_size % num_replicas == 0``;
      it works whenever ``num_replicas * batch_size`` is divisible by
      ``group_size``. In particular, configurations with ``K < W`` (e.g.
      ``W=8, B=2, K=4``) are supported here but rejected by
      ``GroupDistributedSampler``.
    - Different ranks see DIFFERENT prompt-index slices within one
      iteration (load-balanced shuffle of ``groups_per_iter`` groups
      worth of prompt copies), rather than every rank seeing the same
      prompt sequence. Trellis2 cross-GPU upstream sharing only requires
      that all K copies live in the *same* iteration, which is preserved
      regardless of which rank owns which copy.
