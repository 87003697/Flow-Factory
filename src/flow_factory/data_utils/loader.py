...
def _create_or_load_dataset(
    ...
    #    owned by ``dataset_cls`` so the writer and the consolidator cannot drift.
    part_arrow_path = dataset_cls.build_part_arrow_path(
    ...
    _ = dataset_cls(split=split, **kwargs)
    ...
    #    ``dataset_cls.build_part_arrow_path``.
    if is_orchestrator:
        dataset_cls.consolidate_parts(merged_cache_path, num_shards, split=split)
