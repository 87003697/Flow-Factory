...
def _concat_videos_grid(videos: List[np.ndarray]) -> np.ndarray:
    """Concatenate (T,H,W,C) videos into a composite video using image grids."""
    max_frames = max(video.shape[0] for video in videos)
    padded_videos = [
        np.concatenate(
            [video, np.repeat(video[-1:], max_frames - video.shape[0], axis=0)],
            axis=0,
        )  # (max_frames, H, W, C)
    ...
    frames = [
        np.array(
            _concat_images_grid([Image.fromarray(video[frame_idx]) for video in padded_videos])
        )  # (grid_h, grid_w, C)
        for frame_idx in range(max_frames)
    ]
    return np.stack(frames, axis=0)  # (max_frames, grid_h, grid_w, C)
...
@dataclass
class LogTableIncrement:
    """Per-step batch of rows for an incremental (append-only) text table logger.

    Stateless IR: backend loggers (e.g. WandbLogger) keep the cumulative table
    state keyed by the logging key. ``columns`` defines the row dict keys and
    the final table column order; backends may prepend their own index column.
    """
    ...
    def __post_init__(self) -> None:
        if not self.columns:
            raise ValueError("LogTableIncrement requires non-empty columns")
        if "global_step" in self.columns:
            raise ValueError("'global_step' is reserved by WandbLogger; rename your column")
        column_set = set(self.columns)
        for row_idx, row in enumerate(self.rows):
            if set(row.keys()) != column_set:
                raise ValueError(
                    f"row {row_idx} keys {sorted(row.keys())} do not match "
                    f"columns {sorted(column_set)}"
                )

    @classmethod
    def from_reason_samples(cls, samples: List[BaseSample]) -> Optional["LogTableIncrement"]:
        """Build a reason-only snapshot from samples carrying ``reward_reason``."""
        rows: List[Dict[str, Any]] = []
        for idx, sample in enumerate(samples):
            reason = sample.extra_kwargs.get("reward_reason")
            if not isinstance(reason, str) or not reason.strip():
                continue
            rewards = sample.extra_kwargs.get("rewards")
            if isinstance(rewards, float):
                reward_str = f"{rewards:.2f}"
            elif isinstance(rewards, (list, tuple)) and rewards:
                reward_str = (
                    f"{rewards[0]:.2f}"
                    if len(rewards) == 1
                    else ", ".join(f"{r:.2f}" for r in rewards)
                )
            elif isinstance(rewards, dict) and rewards:
                reward_str = (
                    f"{next(iter(rewards.values())):.2f}"
                    if len(rewards) == 1
                    else ", ".join(f"{k}: {v:.2f}" for k, v in rewards.items())
                )
            else:
                reward_str = ""
            rows.append(
        ...
        if not rows:
            return None
        return cls(rows=rows, columns=("sample_idx", "reward", "prompt", "reason"))
...
class LogFormatter:
    ...
    def format_dict(cls, data: Union[Dict, Any]) -> Dict[str, Any]:
        ...
        for k, v in data.items():
            reason_key = f"{k}_reasons"
            if reason_key in clean_data:
                continue
            if not cls._is_sample_collection(v):
                continue
            reason_table = LogTableIncrement.from_reason_samples(v)
            if reason_table is not None:
                clean_data[reason_key] = reason_table
    ...
    @classmethod
    def _process_i2v_samples(cls, samples: List[I2VSample]) -> List[Union[LogVideo, None]]:
        """Handle sample with condition images + generated video, composited side-by-side."""

        def _process_single(sample: I2VSample) -> Optional[LogVideo]:
            if sample.video is None:
                return None
            gen_video = LogVideo(sample.video)
            gen_arr = gen_video.get_numpy()  # (T, H, W, C)
            num_frames, frame_height, frame_width = gen_arr.shape[:3]
            cond_arrs = [
                np.stack(
                    [
                        np.array(
                ...
                for image in _to_pil_list(sample.condition_images)
            ]
            composite = (
                _concat_videos_grid(cond_arrs + [gen_arr]) if cond_arrs else gen_arr
            )  # (T, grid_h, grid_w, C) or (T, H, W, C)
            return LogVideo(composite, caption=_build_sample_caption(sample), fps=gen_video.fps)

        return [_process_single(sample) for sample in samples]

    @classmethod
    def _process_v2v_samples(cls, samples: List[V2VSample]) -> List[Union[LogVideo, None]]:
        """Handle sample with condition videos + generated video, composited side-by-side."""

        def _process_single(sample: V2VSample) -> Optional[LogVideo]:
            if sample.video is None:
                return None
            gen_video = LogVideo(sample.video)
            cond_arrs = [
                LogVideo(cond_video).get_numpy()  # (T, H, W, C)
                for cond_video in _to_video_list(sample.condition_videos)
            ]
            all_arrs = cond_arrs + [gen_video.get_numpy()]
            composite = (
                _concat_videos_grid(all_arrs) if len(all_arrs) > 1 else all_arrs[0]
            )  # (T, grid_h, grid_w, C) or (T, H, W, C)
            return LogVideo(composite, caption=_build_sample_caption(sample), fps=gen_video.fps)
